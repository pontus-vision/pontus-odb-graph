...
public class OSelectExecutionPlanner {
  ...
  private boolean securityPoliciesExistForClass(OIdentifier targetClass, OCommandContext ctx) {
    ...
    // LPPM - clazz.getName() is getting rid of the `quotes`, causing issues where class names have dots in them.
    // targetClass.toString(), in contrast, preserves the `quotes`.
    // return security.isReadRestrictedBySecurityPolicy((ODatabaseSession) db, "database.class." + clazz.getName());
    return security.isReadRestrictedBySecurityPolicy((ODatabaseSession) db, "database.class." + targetClass.toString());
  }
