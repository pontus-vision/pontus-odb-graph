    ...
    try {
      ...
      String topicsDiscussedMeeting1 =
              App.executor.eval("App.g.V().has('Event.Meeting.Form_Id', eq('1'))" +
                      ".properties('Event.Meeting.Discussed_Topics').value()" +
      ...
      String eventMetingDate =
              App.executor.eval("App.g.V().has('Object.Data_Source.Name', eq('SHAREPOINT/P-AND-PD-COMMITTEE-MEETINGS'))" +
                      ".out('Has_Ingestion_Event').out('Has_Ingestion_Event').out('Has_Ingestion_Event').has('Event.Meeting.Form_Id', eq('2'))" +
                      ".values('Event.Meeting.Date').next().toString()").get().toString();
      assertEquals(dtfmt.parse("Tue Dec 07 01:01:01 GMT 2021"), dtfmt.parse(eventMetingDate),
              "P&PD Meeting #2 date.");

      String getObjectDataSourceName =
              App.executor.eval("App.g.V().has('Event.Meeting.Form_Id', eq('3')).in('Has_Ingestion_Event')" +
      ...
      String meeting3Participants =
              App.executor.eval("App.g.V().has('Event.Meeting.Form_Id', eq('3')).values('Event.Meeting.Participants')" +
                      ".next().toString()").get().toString();
      assertEquals("MÔNICA; CEBOLINHA; MAGALI; CASCÃO", meeting3Participants, "The participants of Meeting #3.");

      String meetingSubject =
              App.executor.eval("App.g.V().has('Event.Ingestion.Type', eq('P & PD Committee Meetings')).out('Has_Ingestion_Event')" +
                      ".has('Event.Meeting.Form_Id', eq('2')).values('Event.Meeting.Title').next().toString()").get().toString();
  ...
  public void test00002UpsertSharepointPAndPDCommitteeMeetings() throws InterruptedException {
    try {
      ...
      String countEventMeetings =
              App.executor.eval("App.g.V().has('Event.Ingestion.Type', eq('P & PD Committee Meetings'))" +
                      ".out('Has_Ingestion_Event').has('Metadata.Type.Event.Meeting', eq('Event.Meeting'))" +
