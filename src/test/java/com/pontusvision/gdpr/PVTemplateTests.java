...
//          .id().next().toString();
  ...
  @Test
  public void test00011TemplatePersonNaturalDados() throws InterruptedException {
    try {
...
//      def lawfulBasis  = "LEGÍTIMO INTERESSE DO CONTROLADOR"
      ...
      req.setReportTextBase64(
              Base64.getEncoder().encodeToString((
                      "<html lang=\"pt-br\">\n" +
                              ...
                              "    {% if mainkey.Metadata_Type == \"Evento de Consentimento\" || mainkey.Metadata_Type == \"Endereço\" || mainkey.Metadata_Type == \"Objeto E-mail\" || mainkey.Metadata_Type == \"Objeto Identidade\" %}\n" +
                              "    <h3>{{ pv:t(mainkey.Metadata_Type |replace('.',' ')|replace('_',' ')) }}</h3>\n" +
                              "    {{ \"<table style='margin: 5px'><tr style='border: 1px solid #dddddd;text-align: left;padding: 8px;'><th style='border: 1px solid #dddddd;text-align: left;padding: 8px;'>Tipo de dado</th><th style='border: 1px solid #dddddd;text-align: left;padding: 8px;'>Valor</th></tr>\" }}\n" +
                              "        {% for key, value in mainkey.items() %}\n" +
                              "            {% if key == \"Endereço Completo\" || key == \"Email\" || key == \"ID Valor\" || key == \"Dados de Criação dos Metadados\" || key == \"Estado\" %}\n" +
                              "                {{  \"<tr style='border: 1px solid #dddddd;text-align: left;padding: 8px;'><td style='border: 1px solid #dddddd;text-align: left;padding: 8px;'>%s</td><td style='border: 1px solid #dddddd;text-align: left;padding: 8px;'>%s</td>\" | format (pv:t(key.replaceAll('_',' ')) , value )}} {% endfor %}\n" +
                              "            {{ \"</table>\" }}\n" +
                              "            {% endif %}\n" +
                              "        {% endfor %}\n" +
                              "    {% endif %}\n" +
      ...
      String templateId = "Person_Natural/Dados";

      String contextId = App.g.V().has("Person_Natural_Full_Name", P.eq("LEO MARTINS"))
              .id().next().toString();
      ...
      assertEquals(sortLines(expectedReport), sortLines(report));
